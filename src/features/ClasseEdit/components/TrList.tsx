...
export default function TrList({
  ...
  firstName: string;
  lastName: string;
  dateMonthPayMent: string;
  payMentMonth: string;
  dateBookPayMent: string;
  payMentBook: string;
}) {
  return (
    <tr className="*:py-4 *:px-2">
      <td>
        <b className="text-(--primery) ">{index}</b>
      </td>
      <td>{national_id}</td>
      <td>{firstName}</td>
      <td>{lastName}</td>
      <td>
        <span>{dateMonthPayMent}</span>
      </td>
      <td>
        <span>{payMentMonth}</span>
      </td>
      <td>
        <span>{dateBookPayMent}</span>
      </td>
      <td>
        <span>{payMentBook}</span>
      </td>
      <td>
        <div className="flex justify-between w-full">
          <Link
            to={"#edit_" + national_id}
